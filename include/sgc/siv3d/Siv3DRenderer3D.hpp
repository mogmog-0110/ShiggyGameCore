...
/// @file Siv3DRenderer3D.hpp
/// @brief IRenderer3D の Siv3D 実装
///
/// Siv3Dの3D描画APIを使用して3Dレンダリングを行う。
/// ビュー/プロジェクション行列、ライティング、メッシュ描画を提供する。
///
/// @note このファイルはSiv3D SDKに依存するため、CI対象外。
///
/// @code
/// sgc::siv3d::Siv3DRenderer3D renderer;
...
/// @brief IRenderer3D の Siv3D 実装
///
/// Siv3Dの3D描画機能をラップし、sgcの3Dレンダラーインターフェースに変換する。
class Siv3DRenderer3D : public graphics3d::IRenderer3D
{
public:
	/// @brief ビュー行列を設定する
	/// @param view ビュー行列
		...
		const graphics3d::IMaterial* material = nullptr) override
	{
		(void)mesh;
		(void)material;
		// メッシュの頂点をビュー・プロジェクション変換して描画
		// Siv3D固有の3Dメッシュ描画APIを使用
